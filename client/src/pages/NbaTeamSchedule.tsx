 ...
 * NbaTeamSchedule.tsx
 *
 * Full team schedule page for an NBA team.
 * Accessed via /nba/team/:slug (where slug is the Action Network url_slug).
 *
 * Displays all games for the team with:
 *   - Date, opponent, home/away indicator
 *   - DK NJ Spread (line + juice)
 *   - DK NJ Total (over/under + juice)
 *   - DK NJ Moneyline
 *   - Final score
 *   - Who won (W/L)
 *   - Whether team covered the spread (COV / NO / —)
...
function GameRow({
  ...
  const opponentTeam = NBA_BY_AN_SLUG.get(opponentSlug);
  ...
  // Spread coverage
  const spreadCovered = isHome ? game.homeSpreadCovered : game.awaySpreadCovered;
  const spreadLabel = spreadCovered === null ? "—" : spreadCovered ? "COV" : "NO";
  const spreadVariant: "win" | "loss" | "neutral" = spreadCovered === null ? "neutral" : spreadCovered ? "win" : "loss";
  ...
  // Team's spread line
  const teamSpread = isHome ? game.dkHomeSpread : game.dkAwaySpread;
  const teamSpreadOdds = isHome ? game.dkHomeSpreadOdds : game.dkAwaySpreadOdds;
      ...
      <td className="px-3 py-2.5 text-center">
        <div className="flex flex-col items-center gap-0.5">
          <span className="text-[10px] text-gray-300 font-mono">
            {fmtLine(teamSpread)} <span className="text-gray-600">{fmtOdds(teamSpreadOdds)}</span>
          </span>
...
export default function NbaTeamSchedule() {
  ...
  const atsWins = completedGames.filter((g) => {
    const isHome = g.homeSlug === slug;
    return isHome ? g.homeSpreadCovered === true : g.awaySpreadCovered === true;
  }).length;
  const atsLosses = completedGames.filter((g) => {
    const isHome = g.homeSlug === slug;
    return isHome ? g.homeSpreadCovered === false : g.awaySpreadCovered === false;
  }).length;
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          ...
          <div className="bg-card border border-white/[0.06] rounded-lg p-3 text-center">
            <div className="flex items-center justify-center gap-1 mb-1">
              ...
              <span className="text-[9px] text-gray-500 font-mono uppercase tracking-widest">ATS</span>
            </div>
            <p className="text-lg font-bold text-white font-mono">{atsWins}-{atsLosses}</p>
            <p className="text-[9px] text-gray-600 font-mono">Spread coverage</p>
          </div>
          <div className="bg-card border border-white/[0.06] rounded-lg p-3 text-center">
            <div className="flex items-center justify-center gap-1 mb-1">
              <Calendar className="w-3 h-3 text-gray-500" />
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-white/[0.06]">
                    ...
                    <th className="px-3 py-2 text-center text-[9px] text-gray-600 font-mono uppercase tracking-widest">Spread</th>
