...
function PitcherPanel({ prop, teamAbbrev, side, isRight = false }: PitcherPanelProps) {
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
      ...
      {/* ── Matchup table — mirrors lineup card batting order ── */}
      {matchupRows.length > 0 && (
        <div style={{ padding: "6px 10px" }}>
          {/* Header */}
          <div style={{
            padding: "4px 0 6px 0", borderBottom: "1px solid rgba(24,36,51,0.8)", marginBottom: 2,
            display: "flex", alignItems: "center", gap: 8,
          }}>
            <div style={{ fontFamily: '"Barlow Condensed", sans-serif', fontSize: 9, fontWeight: 700, letterSpacing: "1.5px", textTransform: "uppercase", color: "rgba(255,255,255,0.5)" }}>
              BATTING ORDER
            </div>
            <div style={{ marginLeft: "auto", display: "flex", gap: 16 }}>
              {["K%", "ADJ K%", "xK/PA"].map(h => (
                <div key={h} style={{ fontFamily: '"Barlow Condensed", sans-serif', fontSize: 9, fontWeight: 700, letterSpacing: "1px", textTransform: "uppercase", color: "rgba(255,255,255,0.35)", width: 40, textAlign: "right" }}>
                  {h}
                </div>
              ))}
            </div>
          ...
          {matchupRows.map((row, i) => (
            <div key={i} style={{
              padding: "5px 0",
              borderBottom: i < matchupRows.length - 1 ? "1px solid rgba(24,36,51,0.6)" : "none",
              display: "flex", alignItems: "center", gap: 8,
            }}>
              {/* Batting order number */}
              <div style={{ fontFamily: '"Barlow Condensed", sans-serif', fontSize: 13, fontWeight: 700, color: "#FFFFFF", width: 16, flexShrink: 0, textAlign: "right" }}>
                {row.spot}
              </div>
              {/* Player name */}
              <div style={{ fontFamily: '"Barlow Condensed", sans-serif', fontSize: 15, fontWeight: 800, color: "#FFFFFF", flex: 1, minWidth: 0, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                {row.name}
              </div>
              {/* Bats badge */}
              <div style={{
                fontFamily: '"Barlow Condensed", sans-serif', fontSize: 11, fontWeight: 700,
                letterSpacing: "0.5px", textTransform: "uppercase", color: "#7EB8D4",
                background: "rgba(30,60,90,0.6)", padding: "2px 5px", borderRadius: 3,
                border: "1px solid rgba(30,80,120,0.4)", flexShrink: 0,
              }}>
                {row.bats}
              </div>
              {/* Stats */}
              <div style={{ display: "flex", gap: 0 }}>
                {[
                  row.kRate != null ? `${row.kRate.toFixed(1)}%` : "—",
                  row.adj   != null ? `${row.adj.toFixed(1)}%`   : "—",
                  row.expK  != null ? row.expK.toFixed(3)         : "—",
                ].map((val, j) => (
                  <div key={j} style={{
                    fontFamily: '"Barlow Condensed", sans-serif', fontSize: 11, fontWeight: 700,
                    color: "rgba(255,255,255,0.65)", width: 48, textAlign: "right",
                  }}>
                    {val}
                  </div>
                ))}
      ...
      {Object.keys(signals).length > 0 && (
        <div style={{ padding: "8px 12px 10px", borderTop: "1px solid rgba(24,36,51,0.5)" }}>
          <div style={{ fontFamily: '"Barlow Condensed", sans-serif', fontSize: 9, fontWeight: 700, letterSpacing: "1.5px", textTransform: "uppercase", color: "rgba(255,255,255,0.35)", marginBottom: 6 }}>
            SIGNALS
          </div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
            {Object.entries(signals).map(([key, val]) => {
              const numVal = typeof val === "number" ? val : parseFloat(String(val));
              const isGood = !isNaN(numVal) && numVal > 0;
              return (
                <div key={key} style={{
                  fontFamily: '"Barlow Condensed", sans-serif', fontSize: 10, fontWeight: 700,
                  letterSpacing: "0.5px",
                  color: isGood ? "rgba(255,255,255,0.75)" : "rgba(255,255,255,0.35)",
                  background: isGood ? "rgba(255,255,255,0.07)" : "rgba(255,255,255,0.03)",
                  border: `1px solid ${isGood ? "rgba(255,255,255,0.15)" : "rgba(255,255,255,0.06)"}`,
                  borderRadius: 3, padding: "2px 6px",
                }}>
                  {key.replace(/_/g, " ")}: {!isNaN(numVal) ? numVal.toFixed(3) : String(val)}
                </div>
              );
            })}
          </div>
