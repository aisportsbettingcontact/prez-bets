...
// Cache computed font sizes: key = "text|containerWidth|fontWeight|maxFontPx|minFontPx"
// Capped at 2000 entries to prevent unbounded growth in long sessions.
const _fontSizeCache = new Map<string, number>();
const _FONT_CACHE_MAX = 2000;
...
function computeAutoFontSize(
  ...
  const cacheKey = `${text}|${Math.round(containerWidth)}|${fontWeight}|${maxFontPx}|${minFontPx}`;
  const cached = _fontSizeCache.get(cacheKey);
  ...
  if (_fontSizeCache.size >= _FONT_CACHE_MAX) {
    // Evict oldest entry to keep memory bounded
    _fontSizeCache.delete(_fontSizeCache.keys().next().value!);
  }
  _fontSizeCache.set(cacheKey, chosen);
  return chosen;
}
...
function DesktopMergedPanel({
  ...
  // ── Team colors — pre-loaded by games.list, no per-card round-trip ──────────
  const colors = { away: game.awayColors ?? null, home: game.homeColors ?? null };
        ...
        */}
        <div className="lg:hidden w-full">
          ...
          {mode === "full" && (() => {
            ...
            // ── Edge PP values — pre-computed by the server, not derived on the client ──
            const awaySpreadEdgePP: number = typeof game.awaySpreadEdgePP === 'number' ? game.awaySpreadEdgePP : NaN;
            const homeSpreadEdgePP: number = typeof game.homeSpreadEdgePP === 'number' ? game.homeSpreadEdgePP : NaN;
            const overEdgePP:       number = typeof game.overEdgePP       === 'number' ? game.overEdgePP       : NaN;
            const underEdgePP:      number = typeof game.underEdgePP      === 'number' ? game.underEdgePP      : NaN;
            const awayMlEdgePP:     number = typeof game.awayMlEdgePP     === 'number' ? game.awayMlEdgePP     : NaN;
            const homeMlEdgePP:     number = typeof game.homeMlEdgePP     === 'number' ? game.homeMlEdgePP     : NaN;
            const spreadEdgePP: number = (() => { const a = isNaN(awaySpreadEdgePP) ? -Infinity : awaySpreadEdgePP; const h = isNaN(homeSpreadEdgePP) ? -Infinity : homeSpreadEdgePP; const b = Math.max(a, h); return b === -Infinity ? NaN : b; })();
            const totalEdgePP:  number = (() => { const o = isNaN(overEdgePP)       ? -Infinity : overEdgePP;       const u = isNaN(underEdgePP)      ? -Infinity : underEdgePP;      const b = Math.max(o, u); return b === -Infinity ? NaN : b; })();
            const mlEdgePP:     number = (() => { const a = isNaN(awayMlEdgePP)     ? -Infinity : awayMlEdgePP;     const h = isNaN(homeMlEdgePP)     ? -Infinity : homeMlEdgePP;     const b = Math.max(a, h); return b === -Infinity ? NaN : b; })();
            const bestEdgePP:   number = (() => { const vals = [spreadEdgePP, totalEdgePP, mlEdgePP].filter(v => !isNaN(v)); return vals.length > 0 ? Math.max(...vals) : NaN; })();
