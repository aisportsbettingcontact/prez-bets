...
import { updateBookOdds, listNbaTeams, getNbaTeamByDbSlug, getGameTeamColors, deleteGameById, getFavoriteGameIds, getFavoriteGamesWithDates, toggleFavoriteGame, updateAnOdds, listGamesByDate, listOddsHistory, getBracketGames, auditAndAdvanceAllBracketWinners, getTeamColorsCached } from "./db";
...
// ─── Edge computation (server-authoritative) ──────────────────────────────────
// Moves edge math off the client so thresholds and verdict tiers stay server-side.
function americanToImplied(odds: number): number {
  if (isNaN(odds)) return NaN;
  return odds < 0 ? (-odds) / (-odds + 100) : 100 / (odds + 100);
}

function computeEdgePP(bookOdds: string | null | undefined, modelOdds: string | null | undefined): number {
  const bk = parseFloat(bookOdds ?? "");
  const ml = parseFloat(modelOdds ?? "");
  if (isNaN(bk) || isNaN(ml)) return NaN;
  const bkImpl = americanToImplied(bk);
  const mlImpl = americanToImplied(ml);
  if (isNaN(bkImpl) || isNaN(mlImpl)) return NaN;
  return (mlImpl - bkImpl) * 100;
}

function addEdgeFields<T extends {
  ...
  const awaySpreadEdgePP = computeEdgePP(game.awaySpreadOdds, game.modelAwayPLOdds);
  const homeSpreadEdgePP = computeEdgePP(game.homeSpreadOdds, game.modelHomePLOdds);
  const overEdgePP       = computeEdgePP(game.overOdds,       game.modelOverOdds);
  const underEdgePP      = computeEdgePP(game.underOdds,      game.modelUnderOdds);
  const awayMlEdgePP     = computeEdgePP(game.awayML,         game.modelAwayML);
  const homeMlEdgePP     = computeEdgePP(game.homeML,         game.modelHomeML);
  return { ...game, awaySpreadEdgePP, homeSpreadEdgePP, overEdgePP, underEdgePP, awayMlEdgePP, homeMlEdgePP };
}
  ...
  files: router({
    ...
    upload: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        // Validate filename extension before decoding
        const lowerName = input.filename.toLowerCase();
        if (!lowerName.endsWith(".xlsx") && !lowerName.endsWith(".csv")) {
          throw new TRPCError({ code: "BAD_REQUEST", message: "Only .xlsx and .csv files are accepted" });
        }

        const buffer = Buffer.from(input.contentBase64, "base64");

        // Magic byte validation — verify actual file content matches claimed extension.
        // XLSX files are ZIP archives and start with PK\x03\x04 (0x504B0304).
        // CSV files are plain text — validate they contain no null bytes (binary indicator).
        if (lowerName.endsWith(".xlsx")) {
          const magic = buffer.slice(0, 4);
          if (magic[0] !== 0x50 || magic[1] !== 0x4B || magic[2] !== 0x03 || magic[3] !== 0x04) {
            throw new TRPCError({ code: "BAD_REQUEST", message: "File content does not match .xlsx format" });
          }
        } else {
          // CSV: reject if buffer contains null bytes (strong indicator of binary content)
          if (buffer.includes(0x00)) {
            throw new TRPCError({ code: "BAD_REQUEST", message: "File content does not match .csv format" });
          }
        }
  ...
  games: router({
    ...
    list: publicProcedure
      ...
      .query(async ({ input }) => {
        ...
        // Attach pre-computed edge PP fields + team colors (cached, no per-card round-trips)
        const withEdge = filtered.map(addEdgeFields);
        return Promise.all(withEdge.map(async g => ({
          ...g,
          awayColors: await getTeamColorsCached(g.awayTeam, g.sport ?? 'NCAAM'),
          homeColors: await getTeamColorsCached(g.homeTeam, g.sport ?? 'NCAAM'),
        })));
      }),
