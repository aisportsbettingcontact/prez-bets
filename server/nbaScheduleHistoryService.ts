/**
 * nbaScheduleHistoryService.ts
 *
 * Fetches NBA game schedules and DraftKings NJ (book_id=68) odds from the
 * Action Network v2 scoreboard API, then upserts results into the
 * nba_schedule_history table.
 *
 * ─── Data Source ─────────────────────────────────────────────────────────────
 *   API: https://api.actionnetwork.com/web/v2/scoreboard/nba
 *   Book: DraftKings NJ (book_id=68) — SOLE odds source per user requirement
 *   Markets: markets["68"].event.spread / total / moneyline (dict keyed by book_id)
 *
 * ─── Result Derivation ───────────────────────────────────────────────────────
 *   awaySpreadCovered — away score + spread > home score
 *   homeSpreadCovered — home score - spread > away score (inverse)
 ...
 *
 * ─── Logging Standard ────────────────────────────────────────────────────────
 *   [NbaScheduleHistory][STEP] plain-English description
...
import {
  nbaScheduleHistory,
  type InsertNbaScheduleHistory,
  type NbaScheduleHistoryRow,
} from "../drizzle/schema";
import { and, eq } from "drizzle-orm";

// ─── Constants ────────────────────────────────────────────────────────────────
const TAG = "[NbaScheduleHistory]";
const AN_API_BASE = "https://api.actionnetwork.com/web/v2/scoreboard/nba";
...
export interface NbaScheduleRefreshResult {
  date: string;
  fetched: number;
  upserted: number;
  skipped: number;
...
/**
 * Derive whether the away team covered the spread.
 * away covers if: awayScore + awaySpread > homeScore
 * (e.g. away +11.5: 120 + 11.5 = 131.5 > 132 → NO)
 */
function deriveAwaySpreadCovered(
  awayScore: number | null,
  homeScore: number | null,
  awaySpread: number | null
): boolean | null {
  if (awayScore == null || homeScore == null || awaySpread == null) return null;
...
/**
 * Fetch all NBA games for a single date from the AN v2 API (DK NJ book only).
...
export async function fetchNbaScheduleForDate(
  dateStr: string
): Promise<InsertNbaScheduleHistory[]> {
  ...
  const results: InsertNbaScheduleHistory[] = [];
  let skippedNoTeam = 0;
  let skippedNoDk = 0;
...
/**
 * Upsert a batch of NBA schedule history rows into the DB.
 * Uses anGameId as the deduplication key.
 * On conflict: updates all mutable fields (scores, status, odds, results, lastRefreshedAt).
 */
export async function upsertNbaScheduleHistory(
  rows: InsertNbaScheduleHistory[]
...
/**
 * Refresh NBA schedule history for a single date.
 * Fetches from AN DK NJ API and upserts into nba_schedule_history.
 *
 * @param dateStr - YYYYMMDD format (e.g. "20260410")
 */
export async function refreshNbaScheduleForDate(
  dateStr: string
): Promise<NbaScheduleRefreshResult> {
  console.log(`${TAG}[REFRESH] ════ Starting refresh for date=${dateStr} ════`);

  let rows: InsertNbaScheduleHistory[];
  try {
    rows = await fetchNbaScheduleForDate(dateStr);
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
  ...
  const result: NbaScheduleRefreshResult = {
    date: dateStr,
    fetched: rows.length,
    upserted,
...
/**
 * Backfill NBA schedule history for the last N days (default: 30).
...
export async function backfillNbaScheduleHistory(
  ...
  for (let i = 0; i <= daysBack; i++) {
    ...
    try {
      const result = await refreshNbaScheduleForDate(dateStr);
      results.push(result);
...
/**
 * Get the last N completed games for a specific NBA team (by AN url_slug).
 * Returns games where the team was either away or home.
 * Sorted by gameDate DESC (most recent first).
 *
 * @param teamSlug - AN url_slug, e.g. "boston-celtics"
 * @param limit    - Number of games to return (default: 5 for Last 5 panel)
 */
export async function getNbaLastNGamesForTeam(
  teamSlug: string,
  limit = 5
): Promise<NbaScheduleHistoryRow[]> {
  console.log(
  ...
  const rows = await db
    .select()
    .from(nbaScheduleHistory)
    .where(eq(nbaScheduleHistory.gameStatus, "complete"))
    .orderBy(nbaScheduleHistory.gameDate)
    .limit(300);
...
/**
 * Get the full schedule for an NBA team (all games, any status).
 * Returns games sorted by gameDate DESC (most recent first).
 *
 * @param teamSlug - AN url_slug, e.g. "boston-celtics"
 */
export async function getNbaFullScheduleForTeam(
  teamSlug: string
): Promise<NbaScheduleHistoryRow[]> {
  console.log(
...
/**
 * Get last 5 completed games for both teams in an NBA matchup.
...
export async function getNbaLast5ForMatchup(
  ...
  const [awayLast5, homeLast5] = await Promise.all([
    getNbaLastNGamesForTeam(awaySlug, 5),
    getNbaLastNGamesForTeam(homeSlug, 5),
  ]);
...
/**
 * Compute situational records for an NBA team from their schedule history.
 ...
 */
export async function getNbaSituationalStats(teamSlug: string, limit = 82) {
  console.log(
  ...
  // Helper: is this team the away team in this game?
  const isAway = (g: NbaScheduleHistoryRow) => g.awaySlug === teamSlug;

  // Helper: did this team win?
  const teamWon = (g: NbaScheduleHistoryRow): boolean | null => {
    if (g.awayWon == null) return null;
    return isAway(g) ? g.awayWon : !g.awayWon;
  };

  // Helper: did this team cover the spread?
  const teamCovered = (g: NbaScheduleHistoryRow): boolean | null => {
    if (isAway(g)) return g.awaySpreadCovered ?? null;
    return g.homeSpreadCovered ?? null;
  };

  // Helper: was this team the moneyline favorite? (negative ML = favorite)
  const wasFavorite = (g: NbaScheduleHistoryRow): boolean => {
    const ml = isAway(g) ? g.dkAwayML : g.dkHomeML;
    if (!ml) return false;
    return parseInt(ml, 10) < 0;
  };

  // Helper: was this team the home team?
