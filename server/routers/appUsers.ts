...
// 10 failed attempts per 15-minute window per IP. Successful logins do not
// increment the counter; the counter resets after the window expires.
...
function checkLoginRateLimit(ip: string): void {
  ...
  if (entry.count >= LOGIN_MAX_ATTEMPTS) {
    ...
    throw new TRPCError({ code: "TOO_MANY_REQUESTS", message: `Too many login attempts. Try again in ${waitSec}s.` });
  }
}
...
async function signAppUserToken(userId: number, role: string, tokenVersion: number) {
  ...
  if (isDev) console.log(`[AppAuth] signAppUserToken: userId=${userId} role=${role} tv=${tokenVersion}`);
...
export async function verifyAppUserToken(token: string) {
  try {
    ...
    if (isDev) console.log(`[AppAuth] verifyAppUserToken: userId=${payload.sub} role=${payload.role} tv=${tv}`);
...
export const ownerProcedure = publicProcedure.use(async ({ ctx, next }) => {
  ...
  if (isDev) console.log(`[AppAuth] ownerProcedure: GRANTED — userId=${user.id} username=${user.username} tv=${user.tokenVersion}`);
...
export const appUserProcedure = publicProcedure.use(async ({ ctx, next }) => {
  ...
  if (isDev) console.log(`[AppAuth] appUserProcedure: GRANTED — userId=${user.id} username=${user.username} tv=${user.tokenVersion}`);
  return next({ ctx: { ...ctx, appUser: user } });
});
...
export const appUsersRouter = router({
  ...
  login: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (isDev) console.log(`[AppAuth] login: userId=${user.id} username=${user.username} role=${user.role} tv=${user.tokenVersion} stayLoggedIn=${input.stayLoggedIn}`);
